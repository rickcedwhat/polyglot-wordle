...
export const useFlaggedWords = () => {
  ...
  const generateMarkdownSummary = () => {
    if (flaggedWords.length === 0) return 'No flagged words in discussion list.';
    ...
    flaggedWords.forEach((item, index) => {
      ...
      if (item.pos) md += `   - **POS**: ${item.pos}\n`;
      if (item.d !== undefined) md += `   - **Difficulty**: ${item.d}\n`;
      if (item.def) md += `   - **Definition**: ${item.def}\n`;
      if (item.note && item.note.trim()) md += `   - **Discussion Note**: ${item.note.trim()}\n`;
      md += `\n`;
    });
    return md.trim();
  };
